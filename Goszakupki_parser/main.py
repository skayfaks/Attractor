...
def parse_order_card(line):
    while True:
        try:
            ...
            divs = soup.findAll(
                'div', {'class': "cardMainInfo__section col-12"})
            for div in divs:
                spans = div.findAll('span')
                if spans[0].text.strip() == 'Сроки проведения закупки':
                    end_date_str += spans[0].text.strip() + \
                        ': ' + spans[1].text.strip()
            ...
            for div in divs:
                ...
                if spans[0].text.strip() == 'Организация, разместившая запрос цен':
                    customer_str += spans[0].text.strip() + \
                        ': ' + spans[1].text.strip()
                    ...
                    if file_extension.lower() == '.doc':  #
                        ...
                        if not os.path.isfile(docx_file):
                            ...
                            try:
                                ...
                                print(
                                    'Failed to Convert: {0}'.format(file_path))
                ...
                for row in table.rows:
                    for cell in row.cells:
                        for key_word in keywords_samples:
                            str_samples = check_length_text(
                                cell, key_word, row, str_samples, filename)
                        for key_word in keywords_analogs:
                            str_analogs = check_length_text(
                                cell, key_word, row, str_analogs, filename)
                        for key_word in keywords_delivery_time:
                            str_delivery_time = check_length_text(
                                cell, key_word, row, str_delivery_time, filename)
                        for key_word in keywords_payment_time:
                            str_payment_time = check_length_text(
                                cell, key_word, row, str_payment_time, filename)
                        for key_word in keywords_address:
                            str_address = check_length_text(
                                cell, key_word, row, str_address, filename)
                        for key_word in keywords_divisibility:
                            str_divisibility = check_length_text(
                                cell, key_word, row, str_divisibility, filename)
                        for key_word in keywords_support:
                            str_support = check_length_text(
                                cell, key_word, row, str_support, filename)
...
def find_keywords(para, str_address, str_analogs, str_delivery_time, str_divisibility, str_payment_time, str_samples,
    ...
    try:
        if para.text or not para.text.isspace():
            # образцы
            str_samples = get_paragraphs(
                para, str_samples, keywords_samples, filename)
            # аналоги
            str_analogs = get_paragraphs(
                para, str_analogs, keywords_analogs, filename)
            # срок доставки
            str_delivery_time = get_paragraphs(
                para, str_delivery_time, keywords_delivery_time, filename)
            # срок оплаты
            str_payment_time = get_paragraphs(
                para, str_payment_time, keywords_payment_time, filename)
            # адрес
            str_address = get_paragraphs(
                para, str_address, keywords_address, filename)
            # делимость
            str_divisibility = get_paragraphs(
                para, str_divisibility, keywords_divisibility, filename)
            # обеспечение
            str_support = get_paragraphs(
                para, str_support, keywords_support, filename)
    except Exception as e:
        print(e)
    return str_address, str_analogs, str_delivery_time, str_divisibility, str_payment_time, str_samples, str_support
...
def parse_url(curr_url):
    ...
    add_parse_str_to_list(customer_str, date_str, end_date_str,
                          method_str, nmc_str, platform_str, procedure_str)
